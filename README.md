...
제품 명세 3종을 마크다운으로 옮겨 저장소에 두었습니다. PDF는 grep 도 diff 도
안 되어, 명세가 바뀌었을 때 무엇이 달라졌는지 확인할 방법이 없기 때문입니다.

| 문서 | 위치 |
|---|---|
| KO-Detect Compact QuickGuide (12p) | [`docs/spec/KO-Detect_Compact_QuickGuide최신화.md`](docs/spec/KO-Detect_Compact_QuickGuide최신화.md) |
| Compact 테스트 실행계획서 (9p) | [`docs/spec/KO-Detect_Compact_테스트_실행계획서.md`](docs/spec/KO-Detect_Compact_테스트_실행계획서.md) |
| KO-DETECT PRO 제품소개 (15p) | [`docs/spec/KO-DETECT_PRO_AI_Safety_Inspection.md`](docs/spec/KO-DETECT_PRO_AI_Safety_Inspection.md) |

**명세 대비 격차와 실행 계획은 [`docs/GAP_ANALYSIS.md`](docs/GAP_ANALYSIS.md)** 에 있습니다.
요약하면 — Compact 명세(QuickGuide 5단계 · 테스트계획 6작업)는 전부 이행했고,
PRO 명세 12항목 중 **충족 6 · 부분 3 · 미구현 3** 입니다. 가장 큰 격차는
탐지 유형(자동 검출이 균열 1종)과 대용량 처리 속도이며, 둘 다 검출 엔진을
학습 모델로 교체해야 닫힙니다.

> **폐쇄망(On-Premise) 구동** — 외부 CDN 의존을 걷어내 페이지의 외부 요청이
> 0건입니다. 차트·3D 라이브러리는 `frontend/static/vendor/` 에 내재화되어 있습니다.
> PRO 명세의 1번 강점이 "인터넷 연결 불필요"인데 CDN에 의존하면 설치 즉시
> 화면이 깨지기 때문입니다.
...
## 7. 제품 워크플로우 — 사진그룹 · 도면 · 산출물

KO-Detect Compact QuickGuide의 5단계를 그대로 구현했습니다.

```
새 프로젝트 → 사진 등록 → 사진 그룹 → AI 분석 → 도면 배치 → 보고서 출력
```

| 개념 | 역할 |
|---|---|
| **사진 그룹** | 부재/위치 단위 묶음. 보고서가 그룹 단위로 정리되고 도면 배치의 단위가 된다 |
| **도면** | 외관조사망도의 바탕. PDF·DXF·JPG·PNG 또는 **빈 화면으로 시작** |
| **위치(Spot)** | 도면 위 핀. 그룹을 연결하고 부재·방향·현장 메모를 기록 |
| **직접 손상 입력** | AI가 놓친 균열을 사용자가 그려 넣음 (`source='manual'`) |

### 결과 파일 3종

| 파일 | 형식 | 내용 |
|---|---|---|
| 사진대지 | Word | 그룹별 사진 + 캡션(결함코드·등급·폭·물량·출처) |
| 손상물량표 | Excel | 집계 시트 + 상세 시트(**물량 산출근거** 포함) |
| 외관조사망도 | DXF | 도면틀·위치핀·손상물량표, **결함 유형별 레이어 분리** |

최소 1개를 고르면 ZIP 하나로 묶여 내려옵니다. ZIP에는 생성 조건을 적은
`README.txt`가 함께 들어갑니다 — 나중에 이 파일만 보고도 어떤 범위로 뽑았는지
알 수 있어야 합니다.

**물량은 단위를 섞지 않습니다.** 균열은 연장(m), 면적형 결함은 면적(m²),
개소형은 개소(EA)로 냅니다. 세 가지를 한 칸에 합치면 집계가 불가능해집니다.

> **스케일이 없으면 물량을 내지 않습니다.** 픽셀을 m로 환산할 수 없으면
> "산출 불가"로 적습니다. 임의 가정값으로 채우면 그 숫자가 보고서에 그대로
> 실립니다. 면적형 물량은 `면적률 × 사진이 담은 실제 면적`이라, 접사(0.2mm/px)와
> 원거리 촬영(6mm/px)이 같은 면적률이라도 물량이 700배 차이 납니다. 그래서
> 상세 시트에 **산출근거 열**을 두어 검토자가 숫자를 되짚을 수 있게 했습니다.

**직접 입력한 손상은 재분석해도 보존됩니다.** 제품 문서가 경고하는 "재분석하면
직접 고친 손상은 사라진다" 문제를 `source` 컬럼으로 막았습니다.

### 7.1 이미지 자동 정합 — 수작업 단축의 실제 근거

`backend/app/services/stitching.py` · `backend/app/routers/stitch.py` · 화면: **현장 → 이미지 자동 정합**

한 면을 나눠 찍으면 이웃한 사진이 겹칩니다. 장마다 따로 검출하면 **겹친 영역의
같은 균열이 장마다 다시 계상**되고, 기술자가 눈으로 대조해 지워야 했습니다.
정합해서 한 장으로 만들면 그 대조가 통째로 사라집니다.

```
특징 추출(CLAHE→SIFT) → 쌍 매칭(Lowe+RANSAC 역검증) →
그래프 정합(최대 연결요소 → BFS 변환 누적) → 거리변환 가중 블렌딩 → 통합 검출
```

| 단계 | 방법 | 왜 이렇게 했는가 |
|---|---|---|
| 특징 추출 | CLAHE(clip 3.0, 8×8) 후 SIFT `contrastThreshold=0.008` | 콘크리트 벽면은 표준편차가 10 남짓이라 SIFT 기본값(0.04)에서 **특징점이 0개** 나온다. 실측값이다 |
| 쌍 매칭 | Lowe 비율 + RANSAC 호모그래피 + **역검증** | `H·H⁻¹`로 되돌린 점이 2.0px 안에 들어와야 채택한다. 겉보기 인라이어만 믿으면 틀린 변환이 통과한다 |
| 그래프 정합 | 최대 연결요소만 채택, BFS로 변환 누적 | 겹치지 않는 사진을 억지로 붙이지 않는다. 빠진 사진은 이름으로 보고한다 |
| 합성 | 거리변환 가중 블렌딩 | 이음매에서 밝기가 튀면 그 자리에 없는 균열이 검출된다 |
| 실척 | 면적비로 GSD 전파 + 배율 편차 보고 | 파노라마의 mm/px를 원본에서 물려받되, 얼마나 늘어났는지를 함께 낸다 |

**실측 검증** (합성 표본, 8장 40% 겹침, GSD 0.0897 mm/px)

| 지표 | 값 |
|---|---|
| 배치 | 8/8장, 커버리지 100%, 캔버스 1681×961 |
| 배치 잔차 | 평균 0.0px / 최대 0.1px |
| 배율 편차 | 1.003 (매칭 16쌍, 평균 신뢰도 0.996) |
| **타일별 검출 합계** | **5건** (겹침에 의한 중복 포함) |
| **파노라마 통합 검출** | **2건** |
| 소요 | 16초 |

화면은 이 두 숫자를 나란히 크게 보여줍니다. 정합이 시간을 줄이는 경로는
**검출이 빨라져서가 아니라 확인할 것이 줄어서**이기 때문에, 그 감소분을
주장이 아니라 숫자로 두었습니다.

`replace_defects`를 켜면 원본 사진의 AI 결함을 파노라마 결과로 교체합니다.
**`source='manual'` 결함은 지우지 않습니다** — 자동 처리가 사람의 입력을
지우면 안 됩니다.

| API | 용도 |
|---|---|
| `GET /api/stitch/estimate?group_id=` | 실행 전 사전 점검 — 장수·스케일 누락·예상 소요·차단 사유 |
| `POST /api/stitch/group` | 정합 + 통합 검출. 파노라마를 `source='panorama'` 사진으로 등록 |

한 번에 40장까지 처리합니다. 겹침이 없으면 **정합하지 않고 그 사실을 보고합니다** —
억지로 붙인 파노라마 위에서 잰 폭은 근거가 되지 못합니다.

---

## 8. 시계열 — 균열 진행과 실시간 MTM
...
기능이 수십 개가 되면 좌측 한 줄로는 스캔이 안 됩니다. **상단에서 맥락(대분류)을
고르고 좌측에서 기능을 고르는** 2단 구조라 어떤 기능에도 두 번 안에 닿습니다.
메뉴 구조는 `frontend/static/js/nav.js` 한 곳에서 정의합니다 — HTML에 손으로
적는 순간 관리가 무너지기 때문입니다.

| 상단 (대분류) | 좌측 (기능) |
|---|---|
| **대시보드** | 프로젝트 · 종합 현황 · 시설물 포트폴리오 · 알림 센터 |
| **진단** | 영상 분석 `F1·F2` · 검출 결과 브라우저 · 결함 수동 입력 · 검출 성능 · 스케일 보정 |
| **현장** | 사진 그룹 · 이미지 자동 정합 · 도면·위치 · 점검 회차 |
| **모니터링** | 실시간 계측 `F9` · 3D 디지털 트윈 `F9` · 채널·임계값 |
| **분석** | 균열 진행 · 건축물 건강검진 · 결함 통계 |
| **의사결정** | 유지관리 정책 · CAPA 조치 · 예산 시뮬레이션 |
| **산출물** | 판정서 `F3` · 보고서 빌더 · 법정 제출 `F6` |

`F1~F9` 는 노션 「건축물 안전진단 AI 솔루션 — 도메인 & 기능 총정리」의 기능
번호입니다. 화면과 명세가 같은 이름을 쓰게 하려고 붙였습니다. 화면이 아직 없는
기능은 감추지 않고 **'설계' 태그**로 남깁니다 — 무엇이 있고 무엇이 없는지
보이는 편이 정직합니다.
...
| 화면 | 내용 |
|---|---|
| **프로젝트** | 목록·생성(이름·저장폴더·시설물유형) → 시설물과 첫 회차 자동 생성 |
| **영상 분석** | 원본↔검출 비교 슬라이더 · 중심선 SVG(표↔영상 양방향) · 품질 게이지 · 처리 단계 · 등급 스택바 · 폭 히스토그램 |
| **사진 상세** (모달) | 보기 / **치수 측정** / **원근 보정** / **손상 그리기** 4개 도구 |
| **검출 결과 브라우저** | 다중 드롭 등록 → **AI 일괄 분석 + 진행 표시줄(ETA)** · 상태 색점 |
| **건강검진** | BHI 게이지 · 6계통 레이더 · 적신호 배너 · CAPA 보드 · 금지표현 검사 |
| **이미지 자동 정합** | 사전 점검 · 5단계 진행 · 파노라마 레이어 전환 · 중복 계상 비교 |
| **실시간 계측** | 채널 칩 · 건전성 MTM 캔들 · 경보 · 채널 시계열 (WebSocket 1초) |
| **3D 뷰** | 층별 등급 색상 + 센서 마커, 실시간 상태에 반응 |
| **검출 성능** | F1·MAE·등급일치율 + **폭 구간별 재현율** — 결과 해석의 전제를 먼저 보인다 |
| **산출물** | 사진대지(Word) · 손상물량표(Excel) · 외관조사망도(DXF) → ZIP |
...
```
PDF/                제품 원본 문서 (QuickGuide · 테스트계획서 · PRO 소개)
docs/spec/          명세 마크다운 변환본 + PRO 페이지 이미지
backend/app/        도메인 · 판정 · 서비스 · API
  ...
  services/         vision · timeseries · sensors · deliverables
  routers/          auth · buildings · detect · live · policy · reports · bhc · workspace
...
우선순위 근거는 [`docs/GAP_ANALYSIS.md`](docs/GAP_ANALYSIS.md) §4 에 있습니다.

**1순위 — 검출 엔진 교체** (PRO 격차 1·2·3을 한 번에)
- [ ] 학습 모델 전환 — 자동 탐지 4종(균열·박리/박락·백태·철근노출), 재현율·폭 정밀도
- [ ] 타일 분할 + 배치 추론 — 4000px 원본을 10초 이내로
- [ ] 실촬영 라벨링 데이터 확보 (합성-실사 도메인 격차)

**2순위 — 시계열 자동 매칭**
- [ ] 정합이 만든 공통 좌표계 위에서 회차 간 균열 자동 연결
      ("작년의 미세 균열이 성장했는가"를 사람이 손으로 잇지 않게)

**3순위 — 운영 요건**
- [ ] Smart Filtering (유형·크기·등급) · Focus Mode
- [ ] RBAC — 관리자/점검원/열람자 3역할 + 기관 분리
- [ ] 오프라인 라이선스 — 서명 키 검증, 만료 시 조회·재출력은 유지

**4순위 — 규모 검증·현장 보정**
- [ ] 1,000장 규모 정합 실측 (PRO 문제정의 1,366장 / 39.8GB)
- [ ] 균열 게이지 대조 촬영으로 PSF 계수 현장 보정
- [ ] 실계측기(로거) 연동 — 시뮬레이터를 어댑터 뒤로
